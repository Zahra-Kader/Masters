...
class sparse_2level:
    ...
    def inv(self):
        ...
        #print (tmp,'tmp')    #TMP INCREASES BY 100 WHEN SIGMA DECREASES BY 10, FOR ALL SIGMA
        ...
        #print (tmp2,'tmp2')
        tmp3=many_tri_inv(tmp2) #THIS IS STILL IN REDUNDANT BLOCKS
        tmp4=mult_vecs_by_blocks(self.vecs,tmp3,self.lims) #AT THIS POINT WE HAVE AN ARRAY OF SHAPE OF VECS INPUT ARRAY
...
def get_chisq_dense(g,data,noise,sig,ant1,ant2,scale_fac=1.0,normfac=1.0):
    ...
    apply_gains_to_mat_c(cov.ctypes.data,g.ctypes.data,ant1.ctypes.data,ant2.ctypes.data,n/2,n)
    cov_no_noise=cov.transpose().copy()
    #print (cov[numpy.where(cov!=0)])
    #print (numpy.diag(cov_no_noise),'cov')

    cov=cov_no_noise+noise
    #plt.imshow(noise)
    #plt.colorbar()
    #plt.plot(numpy.diag(noise))
    #plt.show()
    #print (noise)

    cov=0.5*(cov+cov.transpose())
    #print (numpy.where(cov!=0))
    #print (cov[numpy.where(cov!=0)])
    #plt.plot(cov[numpy.where(cov!=0)])
    #plt.show()
    #plt.imshow(cov)
    #plt.colorbar()
    #print ((cov-cov_no_noise).max(),'cov diff')
    #cov_pt0001=numpy.load('cov_pt0001.npy')
    #print ((cov-cov_pt0001).min(),'diff two sigmas') #difference between cov of two different noises
    #print (numpy.diag(cov),'min cov')
    #plt.plot(numpy.diag(cov))
    #plt.show()
    cov_inv=numpy.linalg.inv(cov)
    #plt.imshow(cov_inv)
    #plt.colorbar()
    #plt.plot(numpy.diag(cov_inv))
    #plt.show()
    #print (cov_inv[numpy.where(cov_inv!=0)],'cov inv') #THIS INCREASES BY 100 WHEN SIGMA DECREASES BY 10, FOR ALL SIGMA
    #plt.plot(data)
    #plt.show()
    ...
    #print '%.2E' % Decimal(chisq), numpy.mean(g[0::2]),numpy.mean(g[1::2]), 'chisq dense'
    return chisq
...
def get_gradient_dense(g,data,noise,sig,ant1,ant2,scale_fac=1.0,normfac=1.0):
    ...
    print (-2*grad/scale_fac + normfac*(grad_real+grad_im)/scale_fac,'grad chisq dense')
...
def get_chisq(g,data,mat,ant1,ant2,scale_fac=1.0,normfac=1.0):
    ...
    if do_times:
        ...
        print t2-t1
    mycov_inv=mycov.inv()
    #mycov_inv=numpy.linalg.pinv(mycov)
    #plt.plot(mycov*numpy.ones(len(data)))
    #plt.savefig('mycov_3by3_sig1_noiseless.png')
    #plt.show()
    #plt.loglog(mycov_inv*numpy.ones(len(data)))
    #plt.savefig('mycov_inv_3by3_sig_1e2.png')
    #plt.show()
    if do_times:
        t2=time.time();
        print t2-t1
...
def get_gradient(g,data,mat,ant1,ant2,scale_fac=1.0,normfac=1.0):
    ...
    grad_real=2*(numpy.sum(g[0::2])-nn)/nn
    grad_im=2*numpy.sum(g[1::2])
    #print (-2*grad/scale_fac + normfac*(grad_real+grad_im)/scale_fac, 'get grad')
    chi_sq=get_chisq(g,data,mat,ant1,ant2,scale_fac=1.0,normfac=1.0)
    grad_chi_sq=-2*grad/scale_fac + normfac*(grad_real+grad_im)/scale_fac
    #print (chi_sq/grad_chi_sq,'ratio')
    #print (grad_chi_sq,'grad chi sq')
    return grad_chi_sq
