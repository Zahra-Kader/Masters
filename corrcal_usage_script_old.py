...
for j in range(runs):
    for i in range(len(sim_gains)):
        gvec[j,i]=np.random.normal(0,.0001,2*Ndish)[i]+sim_gains[i]
...
for m in range(runs):
    ...
    fac=1000.0;
    normfac=1.e-10
    asdf=fmin_cg(corrcal2.get_chisq,gvec[m,:]*fac,corrcal2.get_gradient,(data,mat,ant1,ant2,fac))#,gtol=1.e-10,epsilon=1.e-10)
    fit_gains[m,:]=asdf/fac
    '''
    abs_full=np.array([])
    for i in range(2*Ndish):
        if i%2==0:
            abs_full=np.append(abs_full,np.abs(np.complex(fit_gains[m,i],fit_gains[m,1+i])))
...
print (gvec,'sim gains with fluctuation')
print (fit_gains,'fit gains')
print (gain_mean[0::2]-sim_gains[0::2],'diff gains')
