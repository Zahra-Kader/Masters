...
ts_2=h5py.File('/home/zahra/PIPELINE/example_8by8/draco_synthesis_pt1_gainfluc/maps/tstream_2.h5','r')
ag_2=h5py.File('/home/zahra/PIPELINE/example_8by8/draco_synthesis_pt1_gainfluc/rand_gains/amp_pt1_phase_pt1/app_gain_2.h5','r')
rg_2=h5py.File('/home/zahra/PIPELINE/example_8by8/draco_synthesis_pt1_gainfluc/rand_gains/sig_pt12.h5','r')
gn_2=h5py.File('/home/zahra/PIPELINE/example_8by8/draco_synthesis_pt1_gainfluc/rand_gains/ts_final/app_gain_noise_2.h5','r')
...
for j in range(runs):
    for i in range(len(sim_gains)):
        gvec[j,i]=np.random.normal(0,.01,2*Ndish)[i]+sim_gains[i]
...
for ind_run in range(runs):
    ...
    fac=1.e9;
    normfac=1.e-8
    asdf=fmin_cg(corrcal2.get_chisq,gvec[ind_run,:]*fac,corrcal2.get_gradient,(data,mat,ant1,ant2,fac,normfac))
    fit_gains[ind_run,:]=asdf/fac
    abs_full=np.array([])
...
#np.save('std_dev_10000runs_gainamp_pt1_initfluc_pt01_8by8sq',gain_std)
gain_std_singrun=np.load('std_dev_10000runs_gainamp_pt1_initfluc_pt01_8by8sq.npy')*np.sqrt(10000) #taking the std deviations from 1000 runs and multiplying by sqrt(1000) to get std dev for one run

#print (gvec.flatten()[0::2],'sim gains with fluctuation')
#print (fit_gains.flatten()[0::2],'fit gains')
print (fit_gains[0::2],'fit gains')
#print (sim_gains[0::2],'sim gains')
print (np.abs(fit_gains[0::2]-sim_gains[0::2])/sim_gains[0::2],'diff gains')
...
gain_std_singrun_amp=gain_std_singrun[0::2]
...
#plt.errorbar(sim_gains_amp,rec_gains_amp,xerr=None,yerr=gain_std_singrun_amp,linestyle="None")
...
#colour_scatterplot(m,gain_std_singrun_amp)
